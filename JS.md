...
결국, 메소드 이외에 프로퍼티도 클래스 몸체에서 선언될 수 있다는 의미.
...
구현할 대상만 있음.
>객체를 인터페이스 타입(우리가 정의한 인터페이스 타입)을 주어서 선언하게 되면 인터페이스에 정의된 프로퍼티 명 & 자료형을 준수***(타입 체크)***해야만 한다. => 코딩 시 발생할 수 있는 오류 최소화(***일관성 확보***)
...
cf. 인터페이스는 설계/기획(청사진)을 사전에 정의한 개발 방식에 적용할 수 있음.
 ...
 (implement를 하지 않아도 인터페이스에 있는 프로퍼티와 동일 타입으로 인정될 수 있음)
 선택적 구현도 가능 (키워드)

# Generic
***언제 사용?*** ***클래스 선언시점에 타입을 모를 때*** 사용함. 그래서 생성 시점에 타입을 명시한다.
...
# Angular (V.6) [7/9]
프레임워크 사용의 이유 : ***애플리케이션의 구조화, 일반적인 기능 구현, 안정성 확보***

>angular로 개발 시 단점
페이지 전환도 JS로 구현해야 함.
___

"zone.js": "^0.8.26" 중요 라이브러리 : 변화 감지
참고.
"assets": [
              "src/favicon.ico",
              "src/assets"
...
export class AppComponent {
  title = 'app';    // 상태 데이터 (이것으로 뷰를 제어)
}

Welcome to {{ title }}! // ***데이터 바인딩*** (데이터를 뷰-템플릿과 바인딩), 인터폴레이션
***프로퍼티 바인딩*** : 돔 객체의 프로퍼티에 값 할당
***이벤트 바인딩***

component : 결국 재사용 필요성 때문에.
CSS파일도 SCOPE 구분이 어려웠기 때문에 component 개발 방식을 적용할 수 없었음
***component 개발 방식은 결국 HTML 태그를 생성하는 작업이다.***

ng serve는 기본 watch mode
ng serve -o

component 는 module의 일원이어야 한다. 다른 모듈도 import할 수 있음.
component는 tag와 유사하게 사용됨.
root component 최상위 /
root module 최소 1개씩 존재

***Angular 사용의 핵심!***
원하는 뷰를 component 화(customized 태그)해서 HTML에서 그 component를 호출하면 된다.

@Component({    // 데코레이터(함수), ***뒤에는 클래스가 따라와야*** 한다. 뒤의 클래스의 생성자를 조작한다.
  selector: 'app-root',
...
### ***Angular 처리 흐름 (그림) 이해***
>요약
~dist/index.html -> main.ts -> app.module.ts -> app.component.ts -Data Binding> app.component.html -> 최종적으로 index.html에 의해 view 된다.

Step by Step
- ~dist/index.html -> build된 index.html (***customized 태그=selector***가 존재함 + 번들링된 JS파일들 script 태그)
  - main.ts -> bootstrap module(app.module.ts-루트 모듈)을 기동.
    - app.module.ts -> component들을 포함하는 root module. (app.component.ts 기동)
      - app.component.ts -> (Data Binding) 여기에는 index.html에 명시된 태그를 selector로 정의, 템플릿 html과 스타일도 포함  (***decorator***에 메타데이터를 담는다)
      - CLASS following decorator : 이 export할 class에서 작업을 정의
        - app.component.html (template HTML) -> 여기에는 index.html에 있는 태그(예시로는 app-root)를 표시할 내용을 담는다.
          - 최종적으로 ~dist/index.html에 의해 view 된다.



main.ts : 루트 모듈을 기동 시킴 -> root component 기동 시킴 -> 하위 자식 component도 기동됨

## WEB component
4대 기술 스펙 : HTML Template, Shadow DOM, HTML import, customized element

component는 view를 포함한다. view가 없으면 service에 가까움.

독립적, 완결된 view(다른 요소의 도움 없이도 표시될 수 있어야 함)

component의 구조

>***app.component.ts***
@Component(decorator)도 함수이다. 이 데코레이터는 메타데이터를 포함한 객체를 인자로 전달한다.
메타데이터에는 selector, template, style을 포함 이 메타데이터를 뒤에 오는 컴퍼넌트 클래스에 반영.
이후에 오는 클래스 (Component Class)에서 view를 관리하기 위한 로직을 정의한다.

1. Data binding(단방향 바인딩): 컴퍼넌트 클래스에서 HTML 템플릿으로 이동
  데이터 바인딩 시에는 {{}} - 인터폴레이션을 사용하여 컴퍼넌트 클래스의 데이터와 템플릿을 바인딩한다.

(반대 경우는? ***이벤트 발생 시***)
단방향 <-> 양방향 바인딩 ()

component class에서 이벤트 발생 시? 이벤트는 발생 시킬 수 없다.
>***변화가 발생하면 zone.js가 감지한다.***

컴퍼넌트 클래스는 view를 관리하기 위한 템플릿의 상태 관리 역할

***용어와 코드에 익숙해져야 함***
es6 module & ng module 다름

--------------
[] 프로퍼티 바인딩
() 이벤트 바인딩
[()] 양방향 바인딩
change detection은 zone.js가 담당

angular는 DOM에 직접 접근하지 않고 템플릿과 컴퍼넌트 클래스의 관계를 선언하는 방식, 느슨한 방식, 데이터 바인딩 사용

느슨한 방식 : 템플릿의 변경으로 인한 컴퍼넌트 클래스의 변경을 야기하지 않음

모델이 변경된다? 컴퍼넌트 클래스의 프로퍼티 값이 변경된다는 의미

HTML attribute & property
attribute는 html 문서에 존재하는 것으로 변하지 않는다. property는 dom객체에 존재하는 것으로 변할 수 있다. parsing 할 때 HTML 요소 -> DOM 객체, HTML attribute -> DOM 객체 프로퍼티

7/10 러버덕 키워드

SPA
WEB component
  custom element를 생성하기 위한 API의 집합
  custom element는 반복적인 기능/폼 등을 재사용하기 위해서 캡슐화한 형태이다.  이름은 루트 컴퍼넌트의 데코레이터에서 selector로 정의 가능 -> 이걸 HTML 템플릿에서 사용하면 view를 생성 가능

  크게 4가지 기능 (template, shadow DOM, HTML imports, custom element)

-----

# Data Binding

왜 쓰나? 뷰 & 뷰모델 간의 인터랙션을 위해서.
뷰 <-> 모델(광의로는 데이터베이스)


attributes의 property로써의 value (초기 데이터, 변경되지 않음) & 그냥 value (DOM property = ***최신 상태를 나타냄***)

DOM은 이벤트 발생 -> 상태 변경 -> DOM Property 영향

빌트인 구조 디렉티브 (HTML에서 제어문 사용하는 것처럼)

cf. setAttribute method

템플릿은 HTML처럼 보이지만 JS다. 그래서 data binding 시 변수를 참조할 수 있는 것.

인터폴레이션에는 표현식이 와야 하며 표현할 때는 모두 문자열로 표현한다.
객체의 문자열 화 => pipe ( OBJECT | json)

***프로퍼티 바인딩***에는 객체도 값으로 줄 수 있다. (부모-자식 관계에서 빈번히 사용됨)
    ...
    컴파일 후의 차이
    HTML에서 프로퍼티 바인딩은 보이지 않고 어트리븃 바인딩은 표시됨


/<div [class.alert-클래스 이름]="isError-클래스 프로퍼티(조건식)">...</div>

[style.background-color]="'white'" -> 값이냐 변수이냐 여기서는 '값'

이벤트 바인딩
템플릿 -> 클래스 (방향)
$event라고 이벤트 객체를 써야 함
(input), (click)

### 양방향 바인딩
forms module을 import 해야 함 (모듈 파일 - ngmodule)
하나의 프로퍼티에 양방향이 바인딩 된다.
form control 요소에만 사용하자!

*cf. 양방향 바인딩에 대한 full syntax도 있음*

1. ngClass
***여러 클래스를 제어하고자*** 할 때 사용하자. (배열, 객체로 전달 가능)
OR 연산자 사용
사용 예. (클래스가 많을 때)
